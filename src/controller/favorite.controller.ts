...
export async function getByUserIdAndResourceId(req: Request, res: Response): Promise<void> {
    const { userId, resourceId } = req.params;

    try {
        const favorite = await FavoriteRepository.findByUserIdAndResourceId(userId, Number(resourceId));
        if (!favorite) {
